...
* **Componentes de Lista:** `FlatList` Otimizado para renderização de 15 sensores fictícios com identificação única e valores dinâmicos (Temperatura/Pressão) e `ScrollView` horizontal, utilizado no topo para exibição de informações fixas de status da unidade fabril.
